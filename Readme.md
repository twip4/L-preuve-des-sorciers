...
Pour réaliser cette SAÉ, nous avons décidé de choisir le langage Python avec de librairie standard pour que notre projet puisse s'exécuter facilement sur les ordinateurs de l'IUT.
...
Ensuite, ouvrez un invite de commande placer vous dans le répertoire du projet et lancez la commande suivante :
```bash
python main.py
```
...
### Calcul_cout_matrice
La fonction `calcul_cout_matrice` est conçue pour évaluer le coût de parcours d'une grille. Elle calcule le coût minimal de déplacement à travers chaque case de la grille pour atteindre la destination finale. La fonction retourne une matrice de coûts.
...
* **Définition de la valeur de la case de départ** :
   La valeur de la case de départ est définie comme étant la valeur de la case située en bas à droite de la grille. C'est le point d'arrivée.
...
* **Calcul des coûts pour le reste de la grille** : 
   Pour les cases restantes, le coût est déterminé en prenant le maximum entre le coût de la case juste en dessous et celui de la case à droite, puis en y ajoutant la valeur de la case courante. Cela assure que le chemin choisi soit celui qui maximise le coût cumulatif à chaque étape.
* **Retour de la matrice de coût** :
   La fonction retourne ensuite la matrice de coût complétée. Cette matrice peut être utilisée pour tracer le chemin optimal à emprunter pour se déplacer à travers la grille

### Chemin_mana_min
La fonction `chemin_mana_min` est responsable de trouver le chemin qui nécessite le minimum de mana pour qu'un sorcier puisse traverser une grille donnée. Elle utilise les informations de la matrice des coûts pour tracer un chemin optimal à travers la grille.
* **Calcul de la matrice des coûts** :
   La fonction commence par calculer la matrice des coûts en utilisant la fonction `calcul_cout_matrice` vu précedemment. 
* **Initialisation du chemin** :
   Un tableau `chemin` est initialisé pour stocker les coordonnées des cases que le sorcier devra traverser. On récupère les dimensions de la grille `x` pour le nombre de lignes et `y` pour le nombre de colonnes et on initialise les indices de boucles `i` et `j` à 0 pour commencer le tracé du chemin depuis le coin supérieur gauche de la grille.
...
* **Détermination de la direction** :
   À chaque étape, la fonction décide si elle doit se déplacer horizontalement ou verticalement. Elle rompt la boucle si la destination finale est atteinte. Si le sorcier est sur la dernière ligne, il ne peut que se déplacer horizontalement. Si le sorcier est sur la dernière colonne, il ne peut que se déplacer verticalement. Dans les autres cas, la fonction choisit la direction des cases adjacentes avec le coût le plus élevé.
* **Retour du chemin** :
   Enfin, après avoir traversé la grille, la fonction retourne la liste `chemin` qui contient les coordonnées (x, y) des cases formant le chemin optimal.

### Chemin_potion
La fonction `chemin_potion` calcule le chemin optimal qu'un sorcier peut emprunter en utilisant une potion magique dans une grille de jeu. La potion a le pouvoir de transformer la valeur de la case la plus défavorable (la plus basse) en une valeur neutre (zéro), ce qui pourrait potentiellement offrir un meilleur chemin. Voici les étapes détaillées de cette fonction :
...
### Cout_chemin
La fonction `cout_chemin` est une fonction qui calcule le coût total d'un chemin à travers une grille donné. Voici une explication pas à pas de cette fonction :
* **Initialisation du compteur de coût** :
   La fonction démarre avec l'initialisation d'une variable `count` à 0. Cette variable servira à cumuler les valeurs des cases tout au long du chemin parcouru.
* **Parcours des cases du chemin** :
   ...
   Au cours de cette itération, elle additionne la valeur de chaque case rencontrée sur le chemin à la variable `count`. Cette valeur peut être positive ou négative en fonction de la nature de la case : positive ou négative.
...
### Mana_min_requis
La fonction `mana_min_requis` est conçue pour déterminer la quantité minimale de mana requise pour parcourir un chemin spécifique dans une grille. Voici une explication détaillée de son fonctionnement :
...
* **Retour de la quantité minimale de mana requise** :
   Enfin, après avoir parcouru tout le chemin, la fonction retourne `val_min`, qui est la quantité totale de mana minimum nécessaire pour parcourir le chemin sans épuiser le mana.
